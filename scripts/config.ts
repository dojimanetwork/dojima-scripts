...
function writeGethConfig(argv: any) {
    fs.writeFileSync(path.join(consts.configpath, "geth_genesis.json"), consts.geth_genesis_testnet)
...
function writeDojimaConfig(argv: any) {
    fs.writeFileSync(path.join(consts.configpath, "dojima_genesis.json"), consts.dojima_genesis_testnet)
}
